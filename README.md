...
- **Every write is snapshotted first.** See the safety net above. That is not a separate mode, it is the only write path.
...
**Honest limit:** this README makes no speed claim. Prewarming is a real mechanism with a real design (see [`docs/ORACLE.md`](docs/ORACLE.md)), and how much it saves depends entirely on your model, your hardware, and how long you pause. Turn it on and measure it on your own machine.
</details>
...
What is coming, roughly in the order it is likely to land. No dates: this is one person's project
and a date would be a guess dressed up as a promise. Anything already built and merely hidden is
marked, because "un-hide it" and "build it" are very different amounts of work.
...
- `src/electron/main.ts` - the main process: every IPC handler, the run pipeline, providers, routing, the JSON stores, the audit log. Full Node and OS access.
- `src/electron/preload.cts` - the only bridge between main and renderer. Every capability the UI can call is an explicit `contextBridge.exposeInMainWorld` block here.
- `src/electron/builtinRouter.ts` - the in-process Auto Router.
- `src/electron/projectSnapshot.ts` - the write safety net.
- `src/electron/agentTools.ts` - read, list, and targeted edit, with containment and credential-name refusal.
- `src/electron/loops.ts` - the loop decision layer and its governance constants.
- `src/electron/cli.ts` - headless mode.
- `src/renderer/ui/App.tsx` - the React 18 renderer, talking to main exclusively through `window.metisXxx`.
- `src/shared/runtime-contracts.ts` - the types both sides agree on.

## Documentation

- [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) - process model, run pipeline, IPC surface, store keys, where to add things.
- [`docs/SECURITY.md`](docs/SECURITY.md) - threat model, permission system, secrets handling, third-party risk.
- [`docs/PRIVACY.md`](docs/PRIVACY.md) - the plain-language privacy summary.
- [`docs/LOOPS.md`](docs/LOOPS.md) - the loop design, including the governance rules and why silence stops a loop.
...
Pre-release and moving fast. Built by Lachy, solo. `package.json` declares MIT, and a `LICENSE` file still needs to be added to the repo.

If something in this README does not match what the app does, the README is the bug.
