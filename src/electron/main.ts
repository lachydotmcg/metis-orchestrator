...
import {
  OLLAMA_BASE_URL,
  detectOllamaVisionModel,
  listOllamaModels,
  pullOllamaModel,
  resolveConfiguredVisionModel
} from "./ollama.js";
import {
  appendRunToConversation,
  archiveConversation,
...
import {
  ...
  configureProviders,
  deleteSecret,
  effectiveAccountsForProvider,
  fetchJson,
  formatCooldownDuration,
  healthCheckProvider,
  invokeCloudProvider,
  invokeOllamaProviderStream,
  invokeProvider,
  ...
  orderAccountsForRotation,
  providerCooldownUntil,
  readProviderSecret,
  resolveModelRoute,
  setSecret,
  syncProviderCooldownFromAccounts
} from "./providers.js";
import {
  appendAudit,
...
// ---------------------------------------------------------------------------
// Providers — invocation, key pools, cooldowns and secrets moved to
// ./providers.ts (carved 2026-08-20, docs/STRUCTURAL_DEBT.md item 1).
//
// It was three separate ranges of this file, not one: cooldowns and pooling
// near the top, secrets and status in the middle, invocation five hundred
// lines later. Taking only the invocation block measured at 545 lines needing
// 21, because ten of those were the rest of its own subsystem. Whole, it needs
// ten, and all ten are generic utilities rather than provider concerns.
// ---------------------------------------------------------------------------
...
configureProviders({
  ...
  emitStream,
  estimateUsage,
  createThinkTagStreamSplitter,
  splitThinkTaggedOutput,
  sha256,
  // Kept on this side of the seam: a named electron import would make
  // providers.ts unloadable outside the app, and its suite with it.
  safeStorage
});
...
// ---------------------------------------------------------------------------
// Conversations moved to ./conversations.ts (carved 2026-08-20,
// docs/STRUCTURAL_DEBT.md item 1). 531 lines for seven dependencies — the best
// ratio left. What remains in this file is either the runSession hub, which is
// not a seam at 94 dependencies, or electron shell code where a carve would
// mean injecting electron itself one API at a time.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Knowledge Banks moved to ./knowledge.ts (carved 2026-08-20,
// docs/STRUCTURAL_DEBT.md item 1). 480 lines for five dependencies — chosen
// over the Oracle block, which is bigger and needs 23. Measured, not guessed.
// ---------------------------------------------------------------------------

configureKnowledge({
  sha256,
  readConversations,
  readExistingProjectFiles
});
...
// ---------------------------------------------------------------------------
// Routines — moved to ./routines.ts (carved 2026-08-20, docs/STRUCTURAL_DEBT.md
// item 1). The cleanest seam in this file: 257 lines that needed exactly four
// things from here, because deciding WHEN a routine runs is self-contained and
// actually running one is a single call.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Metis Loops — the runtime moved to ./loop-runtime.ts (carved 2026-08-20,
// docs/STRUCTURAL_DEBT.md item 1). The split is now three ways, and each part
// exists for a reason:
//
//   ./loops.ts          what a loop IS — types and pure decision helpers,
//                       deliberately electron-free so cli.ts and gateway.ts
//                       can import it for nothing.
//   ./loop-runtime.ts   what running one DOES — the scheduler, the tick, the
//                       workers. Needs BrowserWindow, so it cannot live in
//                       loops.ts without dragging electron into every
//                       consumer of LoopRecord.
//   here                the thirteen things it still needs from this file.
// ---------------------------------------------------------------------------
...
// The Ollama client moved to ./ollama.ts (carved 2026-08-20,
// docs/STRUCTURAL_DEBT.md item 1). It was sitting under a "Gallery visual RAG"
// banner, which is why no map found it: the measurement counts lines, not
// meanings. Zero dependencies, so it is imported rather than injected — and
// listOllamaModels was being injected into three separate modules purely
// because it lived in a file none of them could import.
...
// ---------------------------------------------------------------------------
// Metis Gateway — the server itself now lives in ./gateway.ts (carved
// 2026-08-20, docs/STRUCTURAL_DEBT.md item 1). What stays here are the two
// dispatch tables, because their 29 members are this file's own functions and
// moving them would have meant moving everything they touch.
//
// The gateway takes its dependencies rather than importing them: importing
// would make main -> gateway -> main, a cycle. Passing the tables as VALUES is
// what collapses that injected surface from 42 entries to 13. Same shape the
// CLI already uses (CliRuntime in ./cli.ts), for the same reason.
// ---------------------------------------------------------------------------
...
// Wired once, at module scope, so nothing can reach a half-built gateway: the
// autostart in app.whenReady() and the metis-gateway:* IPC handlers both call
// into ./gateway.ts and both run after this.
configureGateway({
  reads: GATEWAY_BRIDGE_READS,
  reducers: GATEWAY_BRIDGE_REDUCERS,
  appendAudit,
  readStoreValue,
  writeStoreValue,
  listOllamaModels,
  ...
  stopLoop,
  decidePolicy,
  applySessionRouteOverrides,
  providerFromRoute,
  resolveOverrideModel,
  invokeProvider,
  serveStaticFile,
  providerDefaultModel: (provider) => providerInfo[provider].defaultModel,
  rendererDist: join(__dirname, "../../dist")
});
