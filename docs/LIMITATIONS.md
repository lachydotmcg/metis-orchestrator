...
- **CI now runs the offline suites on every push** (`.github/workflows`), 26 suites via
  ...
  mappers, the loop walkthrough, the think-token ceiling, the per-reader retrieval policy, the loop gate and named outputs, the loops push channel, the rung ledger, the local model brand, cross-conversation memory, and the honesty of this documentation itself. They cover the adversarially-important slices, not the breadth of `src/`.
