...
**Honest limits on the walkthrough:** no snapshot ids, because the id from a generated write reaches the audit log and never the run record, so the file points at the revert control rather than at a specific backup. And helper turns are named but have no row, since a helper runs in its own conversation and is not a turn of the loop, so the cost total understates whenever helpers ran. The file says so rather than presenting a total that looks complete.
