...
**Honest limits.** Parallel helpers arrived in phase 2A: a turn can spawn up to 3, capped at 9 for the loop's lifetime, and a finished helper wakes the sleeping loop. What is still missing is helpers reporting back *mid-flight* — you learn what one did when it finishes, not while it works. The handoff between steps is one hop and only the last one, so a step at the end of a long chain sees its immediate predecessor's output rather than everything the chain has made. A chain is also a ring with no branches: there is no "if this fails, go back two steps", and the only judgement the model gets to steer with is continue-or-stop. `/loop --budget 200k` now caps token spend alongside the iteration and wall-clock caps. A turn that answers inline with its own decision block is still self-graded — the independent judge only runs on the fallback path, which is by construction the path a turn doing real work always takes. And nothing checks that the model driving a loop is capable enough to reliably decide to *stop*, which the design doc asks for and which matters most on small local models. The caps bound the damage; they do not make a 4B model good at knowing when it is done.
