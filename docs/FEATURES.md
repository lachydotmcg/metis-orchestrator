...
Typing any of it renders a live breakdown **inside the prompt box**, along the bottom of the input, naming what each part will do — the parts you typed shown bright and the applied defaults shown muted, so the grammar teaches itself. It has no fill and no border of its own: guidance about what you are writing belongs where you are writing it, not in a banner stacked on the composer, and the only separation it gets is a hairline rule. A malformed flag shows the reason and refuses to send rather than guessing. The parser is shared between the hint and the thing that runs, so the hint cannot promise something that will not happen, and it is tested at 53 out of 53 including every malformed case.
...
**Every continue is judged by a model that did not do the work.** The local rung judges it, not whatever answered the work turn — Metis is the one product that does not have to pay for a second opinion, since the local router is resident and its tokens are free. The judge is told it did not do the work, and it is shown what actually *ran* (exit codes, console errors) rather than only the turn's own account of itself: "I have added the comments" is what a model writes whether or not it did, and an exit code is not. It can answer `BLOCKED` — *what I was shown does not tell me either way* — which outranks `STOP` when a reply contains both, because a judge that said both has told you it could not tell. Its reason becomes the next turn's directive, and the Loops panel names which model decided each turn. With no local model reachable it falls back to the working model and says so on the record rather than passing a self-judge off as independent.

This covers the turns that answer *inline* too. A chat-routed turn — every step of `plan -> draft -> review -> synthesise` — writes its own decision block, and a `continue` it wrote about its own work now gets a second opinion before it buys another turn. Only a continue: stopping is the recoverable direction and `blocked` claims nothing, so re-checking either would double the calls to confirm the answer that was already safe. If the judge disagrees, the loop stops and says *an independent model judged this finished* rather than pretending it stopped itself.

**A loop runs where you started it.** Type `/loop` in a conversation and its turns land in that conversation, appearing as it finishes them: a centred marker (`Loop turn 2 of 5 — fix what fails`) where a user message would be, then an ordinary run card with all the usual detail. The marker is not a bubble on either side, because nobody sent it — Metis woke itself up. What is never stored as your message is the wake prompt itself, which is the goal plus a digest of earlier turns plus the helper list plus the decision protocol; putting that in the chat with your name on it would misrepresent the run more than hiding it did. Started from an empty new session, the loop gets its own thread and the view moves there once its first turn lands.
...
A loop can also be given a chain rather than a single goal: `/loop --steps "read the tests -> fix what fails -> run them again"` walks the steps in order and starts again at the top, and steps joined with `&` run side by side as helpers, with the chain waiting for all of them before it moves on. Each step is handed what the step before it produced, so a chain hands work along instead of being a list of unrelated prompts sharing a timer.

**A step can name what it produces, and a chain can turn back on itself.**
...
Naming makes explicit what the handoff already carried positionally: a step could always see recent outputs, but only as "whatever came before", never as "the draft". A referenced name that nothing earlier declares is refused in the composer rather than becoming a literal `$draf` in a prompt that then quietly does nothing.

**`review x3`** fans one written step into three parallel instances, named `review 1of3` and so on so the panel, the audit log and the next step's digest can tell them apart. Each instance is told it is one of three working on the same material at the same time, that the obvious first reading is already covered, and that finding nothing the others would not is a fine answer — three identical prompts produce three identical readings, which is one reviewer at three times the price. It deliberately does *not* assign each instance a topic: a step is free text, and "check correctness / clarity / completeness" reads well against `review` and is nonsense against `research competitors`. The composer says how many cycles your helper allowance buys (`3 in parallel · 3 cycles`) before you press enter, because running out settles the loop rather than degrading.

The gate is one conditional edge — the "Pass?" diamond. After the named step runs, a model that did **not** do the work is asked PASS or FAIL, and a FAIL sends the chain back to the target instead of forward. It is the first verdict in this grammar that decides *where* rather than only *whether*. **Silence falls through:** an unreadable verdict, an unreachable judge, or a spent attempt allowance all advance the chain normally, so a gated chain is never worse than an ungated one.

**Honest limits.** Parallel helpers arrived in phase 2A: a turn can spawn up to 3, capped at 9 for the loop's lifetime, and a finished helper wakes the sleeping loop. What is still missing is helpers reporting back *mid-flight* — you learn what one did when it finishes, not while it works. The handoff between steps reaches three hops back, not the whole chain, and a step cannot ask for a *specific* earlier step's output except by naming it with `as $name`. A chain carries one conditional edge and is otherwise still a ring: no terminal step, no branch that is itself a chain. And nothing checks that the model driving a loop is capable enough to reliably decide to *stop*, which the design doc asks for and which matters most on small local models. The caps bound the damage; they do not make a 4B model good at knowing when it is done.

When a loop settles it writes **`walkthrough.md`** into the folder it worked in, and the routing trace comes first: for every turn, the difficulty the router judged, the model that served it, tokens in and out, what it cost, and how long it took. Then the files it wrote, then what actually got checked (exit codes and console errors, not prose), then the verdict and its reason. Every agent that leaves a run report can list the files it touched, and every one of them reconstructs that report from its own tool calls afterwards; Metis has the depth judgement and the serving model as typed records the moment the turn ends, so "turn 1 was trivial and ran local for nothing, turn 2 was deep and went to Opus 5" is a read rather than a guess. It is skipped in `plan` mode and when the loop has no project folder, and it never overwrites a `walkthrough.md` it did not write.

It also lists **the backups**: for every turn that wrote files, the snapshot id and the folder holding the originals. That folder is the point rather than the id — the app's revert control undoes the most recent write and nothing older, so for every turn but the last one the path is the way back, and the walkthrough says exactly that instead of implying a one-click undo of any turn.

**Honest limits on the walkthrough:** helper turns are named but have no row, since a helper runs in its own conversation and is not a turn of the loop, so the cost total understates whenever helpers ran. The file says so rather than presenting a total that looks complete.
